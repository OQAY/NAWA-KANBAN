...
export class CommentService {
  ...
  createComment(taskId: string, comment: CreateCommentRequest): Observable<Comment> {
    return this.http.post<Comment>(`${this.apiUrl}/tasks/${taskId}/comments`, comment);
  }

  updateComment(commentId: string, comment: UpdateCommentRequest): Observable<Comment> {
    return this.http.patch<Comment>(`${this.apiUrl}/comments/${commentId}`, comment);
  }
